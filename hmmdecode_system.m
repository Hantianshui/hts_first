function [pStates,pSeq, fs, bs, s_ori] = hmmdecode_system(seq,tr,e,pi,varargin)
...
%     fs(:,count) =  fs(:,count)./s(count);
    s_ori(count) =  sum(fs(:,count));
    fs(:,count) =  fs(:,count)./s_ori(count);
end
...
%       bs(state,count) = (1/s(count+1)) * sum( tr(state,:)'.* bs(:,count+1) .* e(:,seq(count+1))); 
      bs(state,count) = (1/s_ori(count+1)) * sum( tr(state,:)'.* bs(:,count+1) .* e(:,seq(count+1))); 
    end
end
