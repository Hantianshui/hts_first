function [currentState, logP, pTR, vall] = hmmviterbi_system(seq,tr,e,pi,varargin)
...
for count = 2:L
    for state = 1:numStates
        ...
        for inner = 1:numStates 
            %计算前一时刻所有隐状态到这时刻隐状态的转移概率
            val = vOld(inner) + logTR(inner,state);
            valcnt(inner) = vOld(inner) + logTR(inner,state);
            if val > bestVal
                bestVal = val;
                bestPTR = inner;
            end
        ...
        % pTR是前一时刻所有状态转移到该时刻state状态的最大概率时的前一时刻状态序号
        pTR(state,count) = bestPTR;
        % update v v是整个过程的最终概率
        v(state) = logE(state,seq(count)) + bestVal;
        vall(state,count) = v(state);
    end
    %将这一时刻的所有最终概率更新到下一时刻
    vOld = v;
end
