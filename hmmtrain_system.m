function [guessTR,guessE,guessPi,logliks] = hmmtrain_system(seqs,guessTR,guessE,guessPi,varargin)
...
E_all = zeros(numStates,numEmissions);
if ~pseudoEcounts
    pseudoE = E;
end
Pi = zeros(size(guessTR,1),1);
converged = false;
...
for iteration = 1:maxiter
    ...
    for count = 1:numSeqs
        ...
        if baumwelch   % Baum-Welch training
            % get the scaled forward and backward probabilities（这里的前向后向概率都是进行缩放了，并且运算过程中用对数进行运算）
            [~,logPseq,fs,bs,scale] = hmmdecode_system(seq,guessTR,guessE,guessPi);
            bs(isnan(bs)) = 0;
            bs(isinf(bs)) = 1;
...
%                     E_all(k,i) = E_all(k,i) + sum(exp(logf(k,pos)+logb(k,pos)));
                end
            end
            %更新PI
            for k = 1:numStates
                Pi(k) = exp(logf(k,1)+logb(k,1));
            end
%             for k = 1:numStates
%                 Pi(k) = guessPi(k);
%             end
...
%     guessTR = oldGuessTR;
    ...
    % if any rows have zero transitions then assume that there are no transitions out of the state.
    % 
    if any(totalTransitions == 0)
        ...
        %绘制对比图
%     figure;plot(guessE(152,:));
%     title(sprintf("133°姿态角到所有观测状态观测概率分布图（第%d次处理）",iteration));
%     xlabel('观测状态索引值');
%     figure;plot(oldGuessE(152,:));
%     title(sprintf('133°姿态角到所有观测状态观测概率分布图（第%d次处理）',iteration-1));
%     xlabel('观测状态索引值');
%     figure ; pcolor(guessTR);colorbar;
%     title(sprintf('状态转移矩阵分布图（第%d次处理）',iteration));
%     ylabel('隐状态索引值（1-181/0°-180°）');
%     figure;pcolor(oldGuessTR);colorbar;
%     title(sprintf('状态转移矩阵分布图（第%d次处理）',iteration-1));
%     ylabel('隐状态索引值（1-181/0°-180°）');
    % clean up any remaining Nans
    guessTR(isnan(guessTR)) = 0;
    guessE(isnan(guessE)) = 0;
    guessPi(isnan(guessPi)) =0;
    if verbose
        if iteration == 1
